...
public sealed class LocalSettingsStoreTests : IDisposable
{
    ...
    [Fact]
    public async Task ExistingThirtyMinuteRecordingTimeoutIsPreserved()
    {
        ...
        Assert.Equal(30, loaded.MaximumRecordingMinutes);
        Assert.Equal(LocalSettings.CurrentRecordingTimeoutDefaultVersion, loaded.RecordingTimeoutDefaultVersion);

        await store.SaveAsync(loaded);
        var reloaded = await store.LoadAsync();

        Assert.Equal(30, reloaded.MaximumRecordingMinutes);
        Assert.Equal(LocalSettings.CurrentRecordingTimeoutDefaultVersion, reloaded.RecordingTimeoutDefaultVersion);
    }

    [Fact]
    public async Task NewSettingsUseFiveMinuteRecordingTimeoutByDefault()
    {
        var path = Path.Combine(_temporaryRoot, "new-settings.json");
        var store = new LocalSettingsStore(path);
        ...
        Assert.Equal(5, reloaded.MaximumRecordingMinutes);
    }
    ...
    [Fact]
    public async Task LegacyRecordingRootBecomesFirstStorageTargetWithoutMovingFiles()
    {
        Directory.CreateDirectory(_temporaryRoot);
        var path = Path.Combine(_temporaryRoot, "legacy-storage-root.json");
        var legacyRoot = Path.Combine(_temporaryRoot, "legacy-recordings");
        await File.WriteAllTextAsync(path, $$"""
            {
              "recordingRoot": "{{legacyRoot.Replace("\\", "\\\\")}}"
            }
            """);

        var settings = await new LocalSettingsStore(path).LoadAsync();

        var target = Assert.Single(settings.StoragePool.Targets);
        Assert.Equal(StoragePoolOptions.LegacyTargetId, target.Id);
        Assert.Equal(legacyRoot, target.RootPath);
        Assert.True(target.Enabled);
    }

    [Fact]
    public async Task FirstEnabledStorageTargetRemainsTheLegacyRecordingRootProjection()
    {
        var path = Path.Combine(_temporaryRoot, "multi-storage-root.json");
        var store = new LocalSettingsStore(path);
        var settings = new LocalSettings
        {
            RecordingRoot = @"C:\Old",
            StoragePool = new StoragePoolOptions
            {
                Targets =
                [
                    new StorageTarget { Id = "disabled", RootPath = @"D:\Disabled", Priority = 0, Enabled = false },
                    new StorageTarget { Id = "primary", RootPath = @"E:\Primary", Priority = 1, Enabled = true },
                    new StorageTarget { Id = "backup", RootPath = @"F:\Backup", Priority = 2, Enabled = true }
                ]
            }
        };
        ...
        Assert.Equal(@"E:\Primary", loaded.RecordingRoot);
        Assert.Equal(3, loaded.StoragePool.Targets.Count);
    }
