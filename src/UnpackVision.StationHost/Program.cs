...
if (!allowTestInstance)
{
    ...
    storageOptions.StoragePool = localSettings.StoragePool;
    storageOptions.MaximumRecordingMinutes = localSettings.MaximumRecordingMinutes;
...
{
    // The trusted desktop compares this loopback-only startup snapshot with current
    // Windows addresses. A mismatch means Wi-Fi, Ethernet or tethering changed and the
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var items = page.Select(record => ToStationRecordView(record, storageOptions)).ToArray();
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return Results.Created($"/api/v1/records/{record.Id}", ToStationRecordView(record, storageOptions));
});
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return record is null ? Results.NotFound() : Results.Ok(ToStationRecordView(record, storageOptions));
});
    ...
    CancellationToken cancellationToken) =>
{
    ...
    if (record?.VideoPath is not { Length: > 0 } videoPath ||
        !IsPathUnderTrustedRecordingRoots(videoPath, storageOptions) ||
    ...
    CancellationToken cancellationToken) =>
{
    ...
    return Results.Ok(assets.Select(asset => new
    {
        ...
        segmentCount = asset.Segments.Count,
        asset.Integrity,
        asset.FailureReason,
        hasVideo = IsPathUnderTrustedRecordingRoots(asset.VideoPath, storageOptions) && File.Exists(asset.VideoPath)
    }));
});
    ...
    CancellationToken cancellationToken) =>
{
    ...
    if (asset is null ||
        !IsPathUnderTrustedRecordingRoots(asset.VideoPath, storageOptions) ||
