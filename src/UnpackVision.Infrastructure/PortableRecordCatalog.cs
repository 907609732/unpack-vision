...
public sealed class PortableRecordCatalog : IPortableRecordCatalog
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (File.Exists(path))
        {
            ...
            if (manifest is not null && manifest.WorkspaceId != Guid.Empty)
            {
                ...
                {
                    manifest.SchemaVersion = WorkspaceManifest.CurrentSchemaVersion;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var portable = new PortableScanRecord
        {
            ...
            MediaAssets = record.MediaAssets.Select(asset => new PortableMediaAsset
            {
                ...
                RelativeVideoPath = ToPortableRelativePath(asset.VideoPath, asset.RelativeVideoPath),
                Codec = asset.Codec,
                ...
                DurationMilliseconds = (long)asset.Duration.TotalMilliseconds,
                Segments = asset.Segments.Select(segment => new PortableMediaSegment
                {
                    Id = segment.Id,
                    ...
                    RelativeVideoPath = ToPortableRelativePath(segment.VideoPath, string.Empty),
                    StartOffsetMilliseconds = (long)segment.StartOffset.TotalMilliseconds,
                    DurationMilliseconds = (long)segment.Duration.TotalMilliseconds,
                    Recovered = segment.Recovered,
                    CreatedAt = segment.CreatedAt
                }).Where(segment => !string.IsNullOrWhiteSpace(segment.RelativeVideoPath)).ToArray(),
    ...
    private string ToPortableRelativePath(string? absolutePath, string? storedRelativePath)
    {
        ...
        return IsSafeRelativePath(storedRelativePath ?? string.Empty)
            ? storedRelativePath!.Replace('\\', '/')
            : string.Empty;
    }
