...
public sealed class LocalSettingsStore
{
    ...
    {
        // New installations use five minutes through the property initializer. Existing
        // values are operator data: an old value of 30 may have been chosen deliberately,
        // so a version migration must never silently shorten an active business setting.
        ...
        settings.StoragePool ??= new StoragePoolOptions();
        if ((settings.StoragePool.Targets ?? []).Count == 0)
        {
            settings.StoragePool = StoragePoolOptions.FromLegacyRecordingRoot(settings.RecordingRoot);
        }
        else
        {
            settings.StoragePool.Targets ??= [];
            var firstEnabledTarget = settings.StoragePool.Targets
                .OrderBy(target => target.Priority)
                .FirstOrDefault(target => target.Enabled && !string.IsNullOrWhiteSpace(target.RootPath));
            if (firstEnabledTarget is not null)
            {
                // RecordingRoot remains the compatibility projection used by older hosts.
                settings.RecordingRoot = firstEnabledTarget.RootPath;
            }
        }
