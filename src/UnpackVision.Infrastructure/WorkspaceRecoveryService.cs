    ...
    StorageOptions storageOptions) : IWorkspaceRecoveryService
{
    ...
    private ScanRecord ToScanRecord(PortableScanRecord portable)
    {
        ...
        var assets = portable.MediaAssets.Select(asset =>
        {
            var assetId = asset.Id == Guid.Empty
                ? DeterministicGuid($"{portable.Id:D}|{asset.CameraId}|{asset.Role}")
                : asset.Id;
            return new RecordMediaAsset
            {
                Id = assetId,
                ...
                RelativeVideoPath = asset.RelativeVideoPath,
                Codec = asset.Codec,
                EncoderName = asset.EncoderName,
                HardwareAccelerated = asset.HardwareAccelerated,
                WatermarkBurnedIn = asset.WatermarkBurnedIn,
                Width = asset.Width,
                Height = asset.Height,
                FramesPerSecond = asset.FramesPerSecond,
                StartOffset = TimeSpan.FromMilliseconds(asset.StartOffsetMilliseconds),
                Duration = TimeSpan.FromMilliseconds(asset.DurationMilliseconds),
                Segments = asset.Segments.Select(segment => new RecordMediaSegment
                {
                    Id = segment.Id == Guid.Empty
                        ? DeterministicGuid($"{portable.Id:D}|{assetId:D}|segment|{segment.Sequence}")
                        : segment.Id,
                    MediaAssetId = assetId,
                    Sequence = segment.Sequence,
                    StorageTargetId = string.IsNullOrWhiteSpace(segment.StorageTargetId)
                        ? asset.StorageTargetId
                        : segment.StorageTargetId,
                    VideoPath = ResolveRelative(root, segment.RelativeVideoPath) ?? string.Empty,
                    StartOffset = TimeSpan.FromMilliseconds(segment.StartOffsetMilliseconds),
                    Duration = TimeSpan.FromMilliseconds(segment.DurationMilliseconds),
                    Recovered = segment.Recovered,
                    CreatedAt = segment.CreatedAt
                }).Where(segment => !string.IsNullOrWhiteSpace(segment.VideoPath)).ToArray(),
    ...
    /// <summary>
    /// Creates the same consistent SQLite backup used by workspace recovery before a
    /// recording-root migration rewrites persisted absolute media paths.
    /// </summary>
    public Task BackupDatabaseForMigrationAsync(CancellationToken cancellationToken = default) =>
