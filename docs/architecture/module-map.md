...
| Media | Recording/preview ports, camera-capacity contracts, multi-camera asset/segment/gap contracts, synchronized playback alignment, runtime capability policy, local-camera discovery ports and relay authorization | OpenCV/GStreamer/FFmpeg/MediaMTX/ONVIF/SADP/ISAPI transport details | Media-runtime, discovery-security, range, relay, camera and synchronized-playback tests |
| Storage | Ordered recording targets, whole-order allocation, reserve/warning rules and volume identity | Drive probing, directory creation or settings-window controls | Storage-pool monitor, allocator and migration tests |
...
| Settings | Stable camera-rig, preview-layout, storage-pool configuration models and migration intent | Registry/filesystem persistence details | Settings and migration tests |
